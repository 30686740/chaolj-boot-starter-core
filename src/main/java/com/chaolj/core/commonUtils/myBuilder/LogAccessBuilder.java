...
@Slf4j
public class LogAccessBuilder {
    private String from;
    private String channel;
    private String catalog;
    private String body;

    public LogAccessBuilder() {
        this.channel = "AccessLog";
        this.catalog = MyApp.Helper().ConfigHelper().getApplicationName();
    }

    public LogAccessBuilder From(String value) {
        this.from = value;
        return this;
    }

    public LogAccessBuilder Body(String value) {
    ...
    public LogAccessBuilder Body(HttpServletRequest request) {
    ...
    public void Push() {
        try {
            ...
            if (server == null) {
                log.info("LogAccessBuilder.Push，没有找到服务[QueueServer]的提供者，消息推送被忽略！");
                return;
            }
            ...
            if (!retrunDto.isResult()) {
                log.error("LogAccessBuilder.Push，失败！{}，{}，{}，{}，{}", retrunDto.getMessage(), this.channel, this.catalog, this.from, this.body);
            }
        }
        catch (Exception ex)
        {
            log.error("LogAccessBuilder.Push，失败！{}，{}，{}，{}，{}", ex.getMessage(), this.channel, this.catalog, this.from, this.body);
        }
    }
}

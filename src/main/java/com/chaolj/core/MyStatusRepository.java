...
public class MyStatusRepository<TPO, TMapper extends BaseMapper<TPO>, TStatus extends Enum<TStatus>> {
    ...
    public Class<TStatus> _TStatusClazz;

    // 获取状态名称
    public FunctionDelegate1<String, TStatus> _DBPolicy_GetStatusName = m -> DataStatusDto._0_Apply.name();

    // 获取状态
    public FunctionDelegate1<TStatus, TPO> _DBPolicy_GetStatus = m -> Enum.valueOf(_TStatusClazz, DataStatusDto._0_Apply.name());

    // 设置状态
    public ActionDelegate2<TPO, TStatus> _DBPolicy_SetStatus = (m, value) -> { };

    /// <summary>
    /// 检查状态
    /// </summary>
    public void _DBCheckStatus(TPO model, TStatus... statusArray) {
        ...
        var modelStatus = this._DBPolicy_GetStatus.invoke(model);
        var modelStatusName = this._DBPolicy_GetStatusName.invoke(modelStatus);
        if (Arrays.stream(statusArray).noneMatch(s -> StrUtil.equalsIgnoreCase(this._DBPolicy_GetStatusName.invoke(s), modelStatusName)))
        {
            throw new UIException(this._TPOClazz.getName() + "(" + this._DBPolicy_GetId.invoke(model) + ")，当前状态(" + modelStatusName + ")不支持此操作！");
        }
    }
